...
def ping(host):
    ...
    try:
        ...
        # print("Now Ping {0} [{1}] Use 32bit packet ".format(host, dst_addr))
        print("Now test Ping {0} [{1}] ".format(host, dst_addr))
    ...
    for i in range(0,2):
        ...
        # print("T-T: ", times)
        if times >= 0:
            # print("From {0} replay: bit=32 time={1}ms".format(addr, int(times*1000)))
            accept += 1
